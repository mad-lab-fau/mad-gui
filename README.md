...
| Mac OS                 | [Mac OS (64 bit)](https://github.com/mad-lab-fau/mad-gui/releases/download/v0.2.1-beta.1/mad_gui_mac.zip) | <li>Download the file on the left and extract it, for example to a folder called `mad_gui`</li>. Then, right click on the folder `mad_gui` and choose "New Terminal at Folder". Then type `sudo -xvf mad_gui.tgz` and press return. Afterwards, close the terminal. Right click on the file mad_gui_mac and select "Open" and confirm that you want to open the file. Then, it will take up to a minute until the GUI opens. Please also read about example data in the section below this table. <br /><br /> *Note: If your Mac does not allow you to open this file, perform the actions for "If you want to open an app that hasn’t been notarized or is from an unidentified developer" on the [Apple Support Page](https://support.apple.com/en-us/HT202491). Afterwards, try `./mad_gui_mac.exe` again in your terminal.*|
